...
def read_audio(audio_url: str,
               sampling_rate: int = 16000):
    try:
        content = requests.get(audio_url).content
        wav, sr = librosa.load(io.BytesIO(
            content), sr=sampling_rate, mono=False)
    except Exception:
        warnings.warn("Loading audio fails.")
        return {"channel_0": None, "channel_1": None}, None
    ...
    return {"channel_0": channel_0, "channel_1": channel_1}, sr
...
class VADOnnxWrapper():
    ...
    def _validate_input(self, x, sr: int):
        ...
        if len(x.shape) > 2:
            raise ValueError(
                f"Too many dimensions for input audio chunk {x.shape}")
...
if __name__ == "__main__":
    sampling_rate = 16000
    audio = read_audio("mandarin_mix.wav")
    vad_model = VADOnnxWrapper("WhisperExecutor/models/silero_vad.onnx")

    speech_timestamps = get_speech_timestamps(
        audio["channel_0"], vad_model,
